    ...
    def test_railway_database_url_is_normalized_for_psycopg2(self):
        ...
        self.assertEqual(
            normalize_database_url(
                f"postgres://{username}:{password}@db:5432/name"
            ),
            f"postgresql+psycopg2://{username}:{password}@db:5432/name",
        )
...
class ContactSecurityTests(unittest.TestCase):
    def test_csrf_token_is_signed_expiring_and_secret_specific(self):
        ...
        token = new_csrf_token(signing_key, now=int(time.time()))
        self.assertTrue(verify_csrf_token(token, signing_key))
        self.assertFalse(verify_csrf_token(token, wrong_key))
        old = new_csrf_token(signing_key, now=int(time.time()) - 3601)
        self.assertFalse(verify_csrf_token(old, signing_key))

    def test_keyed_hash_does_not_store_raw_identifier(self):
        digest = keyed_digest("203.0.113.5", secrets.token_urlsafe(32))
    ...
    def test_contact_fields_are_normalized_and_bounded(self):
        clean = validate_contact(
            ContactInput(
                ...
                website="",
                turnstile_token=secrets.token_urlsafe(24),
            )
        )
...
class ContactMailTests(unittest.TestCase):
    ...
    def test_resend_api_receives_bearer_auth_and_reply_to(self, post):
        ...
        self.assertEqual(request.kwargs["headers"]["Authorization"], f"Bearer {api_key}")
